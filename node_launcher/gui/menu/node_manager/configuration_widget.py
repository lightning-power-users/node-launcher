...
class ConfigurationWidget(QTableWidget):
    ...
    def update_key(self, node_name: str, key: str, new_values: List[str]):
        existing_value_items = self.get(node_name, key)
        for row_index, value in existing_value_items:
    ...
    def handle_cell_change(self, row: int, column: int):
        ...
        node_item = self.item(row, 0)
        key_item = self.item(row, 1)
        if node_item is None or key_item is None:
            return
